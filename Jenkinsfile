    ...
    stages {        
        ...
        stage('Deploy') {       
            steps {
                ...
                sh 'pwd && ls -al && /var/jenkins_home/sam/venv/bin/sam deploy -t .aws-sam/build/template.yaml --config-file samconfig.toml --no-confirm-changeset --no-fail-on-empty-changeset'
            }
        }
    }
}